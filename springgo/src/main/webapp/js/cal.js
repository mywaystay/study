...
(function($) { // create private scope (inside you can use $ instead of jQuery)
			...
			for (var i = 0; i < numdays; i++) {
				...
				if ( 
					...
					cell
						...
						.click(function () {
							window.chosenDateObj = new Date(jQuery("select[name=year]", datepicker).val(), jQuery("select[name=month]", datepicker).val(), jQuery(this).text());
	...
	jQuery.fn.simpleDatepicker.formatOutput = function (dateObj) {
		if((dateObj.getMonth() + 1)<10){
			var monthObj = '0'+(dateObj.getMonth() + 1);
		}else{
			var monthObj = (dateObj.getMonth() + 1);
		}
		if(dateObj.getDate()<10){
			var dayObj = '0'+dateObj.getDate();
		}else{
			var dayObj = dateObj.getDate();
		}
		return  dateObj.getFullYear() + monthObj + dayObj;	
	};
