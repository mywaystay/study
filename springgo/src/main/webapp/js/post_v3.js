...
function do_submit(formObj){
	...
	alert("text" + PostEditor.getEditorContentText());
	alert("html" + PostEditor.getEditorContentHTML());
...
function pasteAll()
{
	...
	if (data.indexOf("[^^]\n")>-1)
	{
		...
		PostEditor.setEditorContent(content);
...
function copyAll()
{
	try
	{
		...
		copytext+=PostEditor.getEditorContentHTML()+"[^^]\n";
...
{
	var uploadhtml = "";	
	var uploadhtml=PostEditor.getEditorContentHTML();
